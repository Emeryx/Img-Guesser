...
export class GameCreationService {
    ...
    async createGameSession(roomCode: string, players: Player[], roundTime: number, roundAmount: number, currentRound: number, gameState: GameState): Promise<GameSession> { // The type Promise<GameSession> means that the method returns a promise that then resolves into a GameSession object. The use of an asynchronous function allows to do stuff in the mongoDB and catch potential errors easily
        const newGameSession = new this.gameSessionModel({ roomCode, players, roundTime, roundAmount, currentRound, gameState});
        try {
