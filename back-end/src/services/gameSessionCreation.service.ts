...
export class GameCreationService {
    ...
    async createGameSession(hostName: string, hostImage: string, roundTime: number, roundAmount: number): Promise<GameSession> { // The type Promise<GameSession> means that the method returns a promise that then resolves into a GameSession object. The use of an asynchronous function allows to do stuff in the mongoDB and catch potential errors easily
        const newGameSession = new this.gameSessionModel({
            roomCode: RoomCodeGenerator(),
                players: [{uid: uuidv4(), name: hostName, image: hostImage, score: 0, ready: false, isHost: true}],
                roundTime: roundTime,
